var MarkdownCaptureWebpage = (() => {
  ...
  function webpageMarkdownFilename(title) {
    const safe = cleanText(title, "webpage").replace(/[<>:"/\\|?*\x00-\x1F]/g, "").replace(/[. ]+$/g, "").slice(0, 120).trim();
    return `${safe || "webpage"}.md`;
  }
  function normalizeContentUrls(root2, baseUrl) {
    root2.querySelectorAll("a[href]").forEach((link) => {
      link.setAttribute("href", absoluteUrl(link.getAttribute("href"), baseUrl));
    });
    root2.querySelectorAll("img[src]").forEach((image) => {
...
`;
  ...
  function canonicalUrl(document2, fallbackUrl) {
